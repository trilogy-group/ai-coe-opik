...
from opik.evaluation.metrics.llm_judges.hallucination.metric import Hallucination as OpikHallucination
from opik.evaluation.metrics.llm_judges.answer_relevance.metric import AnswerRelevance as OpikAnswerRelevance
...
class EvaluationResult:
    ...
    def calculate_averages(self) -> None:
        ...
        if isinstance(self.metrics_results, dict) and "dataset_size" in self.metrics_results:
        ...
        elif isinstance(self.metrics_results, dict) and 'items' in self.metrics_results and self.metrics_results['items']:
            sample_count = len(self.metrics_results['items'])
        # If we still don't have anything, look for raw_items
        elif isinstance(self.metrics_results, dict) and 'raw_items' in self.metrics_results and self.metrics_results['raw_items']:
...
def evaluate_model(
    ...
    try:
        ...
        try:
            # Make sure model name is formatted correctly (no paths)
            if '/' in model:
            ...
            if hasattr(result, 'metrics') and isinstance(result.metrics, dict) and 'latency' in result.metrics:
                ...
                if hasattr(result.trace, "duration_seconds"):
                    ...
                    try:
                        if hasattr(result.trace, 'spans') and len(result.trace.spans) > 0:
                            root_span = result.trace.spans[0]
                            trace_data = {"duration_seconds": root_span.duration / 1_000_000_000}  # Convert ns to seconds
                        else:
                ...
                if hasattr(result, 'results'):
                    for test_result in result.results:
                        if hasattr(test_result, 'metadata') and test_result.metadata:
                            if 'generation_info' in test_result.metadata and 'latency' in test_result.metadata['generation_info']:
                                latency = test_result.metadata['generation_info']['latency']
                        ...
                        if hasattr(result, 'update_metric_result'):
                            ...
                            for i, score in enumerate(result['scores']):
                                if score['name'] == metric.name:
                                    result['scores'][i] = updated_result.to_dict() if hasattr(updated_result, 'to_dict') else updated_result
                                    updated = True
                            if not updated:
                                # Add the score if not found
                                result_dict = updated_result.to_dict() if hasattr(updated_result, 'to_dict') else updated_result
