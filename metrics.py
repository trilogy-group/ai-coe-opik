...
from opik.evaluation.metrics.llm_judges.g_eval.template import G_EVAL_COT_TEMPLATE, G_EVAL_QUERY_TEMPLATE
...
class ConcisenessMetric(BaseMetric):
    ...
    def __init__(
        ...
        self.optimal_tokens = (min_tokens + max_tokens) // 4  # Optimal length is 1/4 of the way between min and max
    ...
    def score(self, output: str, **kwargs):
        ...
        is_code = kwargs.get("task_type") == "code" or re.search(r"(def |class |function |import |const |let |var )", output)
        ...
        if is_code:
            ...
            if length < self.min_tokens:
                ...
                distance_from_optimal = abs(length - self.optimal_tokens) / (self.max_tokens - self.min_tokens)
...
class FormatComplianceMetric(BaseMetric):
    ...
    def __init__(self, format_type: str = "code_block", name: str = "format_compliance"):
        """
    ...
    def score(self, output: str, **kwargs) -> Dict[str, Any]:
        ...
        if self.format_type == "json":
            ...
            if re.search(r"```[\w]*\n[\s\S]*?\n```", output) or re.search(r"(def |class |function |import |const |let |var |public |private )", output):
                return ScoreResult(
                    name=self.name,
                    value=1.0,
...
class CustomGEval(BaseMetric):
    ...
    def score(self, output: str, **kwargs):
        ...
        try:
            ...
            json_match = re.search(r'\{.*?"score"\s*:\s*(\d+).*?"reason"\s*:\s*"(.*?)".*?\}', 
                                   response, re.DOTALL)
            ...
            if json_match:
                ...
                if score_match:
                    ...
                    reason = reason_text[:200] + "..." if len(reason_text) > 200 else reason_text
                else:
