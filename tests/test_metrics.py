...
def test_format_compliance_json():
    ...
    assert result["score"] == 1.0, "JSON in code block should be correctly identified after cleaning"
...
def test_task_specific_metrics():
    ...
    qa_metrics = [m["name"] for m in config_data["metrics"] 
    ...
    assert "code_quality" not in qa_metrics, "QA metrics should not include code_quality"
    
    # Test code task metrics
    code_metrics = [m["name"] for m in config_data["metrics"] 
...
def test_export_json_format():
    ...
    assert "metrics_average_scores" in parsed_json[0], "JSON should include metrics scores"
    assert parsed_json[0]["metrics_average_scores"]["conciseness"] == 0.8, "Metric scores should be preserved"
...
def test_argument_parsing():
    ...
    def create_test_parser():
        ...
        parser.add_argument(
            "--type", "-t", choices=["prompts", "models"], required=True, 
            help="Type of experiment"
        )
        parser.add_argument(
            "--task", choices=["qa", "summarization", "code", "chat", "all"],
    ...
    args = parser.parse_args(["--name", "test-exp", "--type", "prompts", "--task", "code"])
...
def test_error_handling_in_evaluation():
    ...
    assert success_result.metrics_average_scores == {}, "No metrics should be calculated yet"
...
def test_dataset_handling():
    ...
    assert len(limited_samples) == sample_count, f"Should limit to {sample_count} samples"
...
def test_metric_combination():
    ...
    assert abs(combined_score - expected_score) < 0.001, "Combined score calculation should be correct"
